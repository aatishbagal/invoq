...
| At audit time, the documented execution settings for confirmation, explanations, and privilege bypass were loaded but not consulted by execution. This created misleading security/configuration semantics. | Medium | `src/invoq/config.py:20-25,73-82`, `src/invoq/core/executor.py:89-178` (audit-time locations) | Resolved by R.5: remove the unused execution section from the schema, defaults, and documentation; reject legacy configs with migration guidance. See [configuration migration](../configuration.md). No privilege-bypass capability is implemented. |
